...
SELECT name, SUM(num_cupcakes) FROM cupcakes LEFT JOIN orders ON (cupcakes.id=orders.cupcake_id) GROUP BY name ORDER BY name;
...
SELECT email FROM customers JOIN orders ON (customers.id=orders.customer_id) WHERE num_cupcakes >= 100 ORDER BY email DESC;
...
SELECT fname, lname, email FROM customers JOIN orders ON (customers.id=customer_id) WHERE cupcake_id = (SELECT cupcakes.id FROM cupcakes WHERE name = 'funfetti') AND processed = True GROUP BY customers.id;